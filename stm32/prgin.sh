...
openocd -f interface/stlink-v2.cfg -f target/stm32f1x_stlink.cfg  #wywołanie OpenOcd, inicjacja połączenia, TUTAJ ZMIENIAJ: programator oraz docelowy µC
}
...
sleep 1 
 ...
 sleep 1
 echo $AA
 sleep 4                    # czekaj 4 sekundy, aż zostanie wgrany program
 echo "resume"
 sleep 1
 echo "reset"
 echo "exit"
...
case $2 in
hex) AA="flash write_image erase "$1"" ;;
bin) AA="flash write_image erase "$1" "$MEMADDR" bin" ;;
*) hlp
   exit 0 ;; 
esac
