...
ZMNN="avr-gcc -O -mmcu=atmega16 "$1" -o program.hex"  # <<-- Tu edytuj typ mikrokontrolera (domyślnie "atmega16")
...
echo `avrdude -c usbasp -p m16 -U flash:w:program.hex -U lfuse:w:0xff:m -U hfuse:w:0xd9:m` # <<-- Tu edytuj typ mikrokontrolera("m16") i port COM: ("/dev/ttyS0")
